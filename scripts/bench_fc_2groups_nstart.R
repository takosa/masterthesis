...
all_results <- mclapply(seq_len(p$nsim), function(s) {
  ...
  result$mbcluster2_nstart5 <- run_mbcluster(counts = tcc$count, group = tcc$group$group, k = 2, nstart = 5)
  # MBCluster.Seq
  cat("start MBCluster.Seq, k = 3\n")
  result$mbcluster3_nstart5 <- run_mbcluster(counts = tcc$count, group = tcc$group$group, k = 3, nstart = 5)
  # MBCluster.Seq
  cat("start MBCluster.Seq, k = 4\n")
  result$mbcluster4_nstart5 <- run_mbcluster(counts = tcc$count, group = tcc$group$group, k = 4, nstart = 5)
  # MBCluster.Seq
  cat("start MBCluster.Seq, k = 5\n")
  result$mbcluster5_nstart5 <- run_mbcluster(counts = tcc$count, group = tcc$group$group, k = 5, nstart = 5)
  ## MBCluster.Seq
